...
class CoverSplit extends \ReyCore\Elementor\WidgetsBase {
	...
	public function __construct( $data = [], $args = null ) {
		...
		if( ! empty($data) ):
			...
			if ( isset($data['settings']) && ($settings = $data['settings']) ) {
				if( ! (isset($settings['main_slide']) && '' === $settings['main_slide']) ){
					add_filter('body_class', function($classes){
						$classes['cover_top'] = '--cover-split-header';
						if( reycore__get_option( 'header_position', 'rel' ) !== 'rel' ){
							$classes['split_top'] = '--cover-split-header--top';
						}
						return $classes;
					});
				}
	 ...
	 */
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Slides Caption Styles', 'rey-core' ),
				'tab' => \Elementor\Controls_Manager::TAB_STYLE,
			]
		);
	...
	public function get_main_image_position($settings){

	}

	public function render_main_image($settings){

		$desktop_class = $tablet_class = [];

		if( isset($settings['main_background_image_mobile']) && $main_bg_img_mobile = $settings['main_background_image_mobile'] ){
			echo reycore__get_attachment_image( [
				'image' => $main_bg_img_mobile,
				'size' => isset($main_bg_img_mobile['size']) ? $main_bg_img_mobile['size'] : 'full',
				'attributes' => [
					'class' => 'cSplit-slide--mainImg --dnone-md --dnone-lg',
				],
			]);
			$desktop_class[] = '--dnone-sm';
			$tablet_class[] = '--dnone-sm';
		}

		if( isset($settings['main_background_image_tablet']) && $main_bg_img_tablet = $settings['main_background_image_tablet'] ){
			echo reycore__get_attachment_image( [
				'image' => $main_bg_img_tablet,
				'size' => isset($main_bg_img_tablet['size']) ? $main_bg_img_tablet['size'] : 'large',
				'attributes' => [
					'class' => implode(' ', array_merge($tablet_class, [
						'cSplit-slide--mainImg',
						'--dnone-lg',
					]))
				]
			]);
			$desktop_class[] = '--dnone-md';
		}

		if( isset($settings['main_background_image']) && $main_bg_img = $settings['main_background_image'] ){
			echo reycore__get_attachment_image( [
				'image' => $main_bg_img,
				'size' => isset($main_bg_img['size']) ? $main_bg_img['size'] : 'medium',
				'attributes' => [
					'class' => implode(' ', array_merge($desktop_class, [
						'cSplit-slide--mainImg',
						'cSplit--abs'
					])),
				]
