	...
	*/
	...
	public function item_template(){
		...
		if( ! reycore_wc__is_catalog()){
			...
			$cart_button .= '<div class="rey-wishlistItem-atc" data-rey-tooltip="{{{data.ob[i].add_to_cart_text}}}" data-rey-tooltip-source="wishlist-atc" {{{ttFixed}}}>{{{data.ob[i].add_to_cart}}}</div>';
			$cart_button .= '<# } #>';
		} ?>
