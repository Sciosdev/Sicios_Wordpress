...
if (!class_exists('REYAJAXFILTERS_CustomFieldsACF_Filter_Widget')) {
	class REYAJAXFILTERS_CustomFieldsACF_Filter_Widget extends WP_Widget {
		...
		public function widget($widget_args, $instance) {
			...
			$args = [
				'url_array'              => $url_array,
				'data_key'               => $base::CF_KEY . $field_key,
			...
			foreach( get_posts($query_args) as $id) {
				...
				$meta_slug = sanitize_title($meta_value);
				...
				if( isset($filters['cf'][$field_key]['terms']) && in_array( $meta_slug, $filters['cf'][$field_key]['terms'], true ) ){
					$li_classes .= 'chosen';
				}
			...
			if ( ! empty($list_items) )
			{
				...
				if( (bool) $instance['rey_multi_col']  ){
					...
					$before_widget .= $base::multicols_css();
				}
