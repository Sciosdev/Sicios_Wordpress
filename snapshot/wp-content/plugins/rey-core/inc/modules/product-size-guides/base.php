...
class Base extends \ReyCore\Modules\ModuleBase {
	...
	public function init() {
		...
		add_action( 'save_post', [$this, 'flush_transient'], 20, 2 );
		add_action( 'delete_post', [$this, 'flush_transient'], 20, 2 );
	...
	public function get_all_guides(){
		...
		if( false !== ($all_guides = get_transient(self::TRANSIENT_NAME)) ){
		...
		set_transient(self::TRANSIENT_NAME, $guides, WEEK_IN_SECONDS);
	...
	public function get_guide_id( $product_id = null ){
		...
		if( $product_display = get_field('pdp_size_guide_display', $product_id) ){
			if( 'show' === $product_display ){
				if( $product_display_custom = get_field('pdp_select_size_guide', $product_id) ){
	...
	public function get_size_guide_content( $id = null ){
		...
		if( ( $document = \Elementor\Plugin::$instance->documents->get( $post->ID ) ) && $document->is_built_with_elementor() ){
			...
			$content = sprintf('<div class="__content">%s</div>', reycore__parse_text_editor($post->post_content));
		}
	...
	public function flush_transient($post_id, $post){
		...
		\ReyCore\Helper::clean_db_transient( implode('_', [ \ReyCore\Ajax::AJAX_TRANSIENT_NAME, self::AJAX_EVENT, $post_id ] ) );
	}

	public static function get_default_guide(){
		return get_theme_mod('pdp_size_guides', '');
	}
