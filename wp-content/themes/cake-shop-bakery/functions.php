...
function cake_shop_bakery_widgets_init() {
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Footer Column 4', 'cake-shop-bakery' ),
		'id'            => 'cake-shop-bakery-footer4',
		'description'   => '',
		'before_widget' => '<aside id="%1$s" class="widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h5 class="footer-column-widget-title">',
...
function cake_shop_bakery_preloader() {
  ...
  $cake_shop_bakery_preloader2_dot_color = get_theme_mod('cake_shop_bakery_preloader2_dot_color');
  ...
  if(get_theme_mod('cake_shop_bakery_logo_max_height') == '') {
	...
	if(get_theme_mod('cake_shop_bakery_preloader_dot_1_color') == '') {
		$cake_shop_bakery_preloader_dot_1_color = '#ffffff';
	}
	if(get_theme_mod('cake_shop_bakery_preloader_dot_2_color') == '') {
		$cake_shop_bakery_preloader_dot_2_color = '#f10026';
	}
	$cake_shop_bakery_theme_color_css = '
		...
		.load hr {
			background-color: '.esc_attr($cake_shop_bakery_preloader2_dot_color).';
		}
	';
...
function cake_shop_bakery_files_setup() {
	...
	if ( ! defined( 'CAKE_SHOP_BAKERY_LIVE_DEMO' ) ) {
		define('CAKE_SHOP_BAKERY_LIVE_DEMO',__('https://demo.themagnifico.net/cake-shop-bakery/','cake-shop-bakery'));
	}
	if ( ! defined( 'CAKE_SHOP_BAKERY_GET_PREMIUM_PRO' ) ) {
		define('CAKE_SHOP_BAKERY_GET_PREMIUM_PRO',__('https://www.themagnifico.net/products/cake-shop-wordpress-theme','cake-shop-bakery'));
	}
	if ( ! defined( 'CAKE_SHOP_BAKERY_PRO_DOC' ) ) {
		define('CAKE_SHOP_BAKERY_PRO_DOC',__('https://demo.themagnifico.net/eard/wathiqa/cake-shop-bakery-pro-doc/','cake-shop-bakery'));
	}
	if ( ! defined( 'CAKE_SHOP_BAKERY_FREE_DOC' ) ) {
		define('CAKE_SHOP_BAKERY_FREE_DOC',__('https://demo.themagnifico.net/eard/wathiqa/cake-shop-bakery-free-doc/','cake-shop-bakery'));
	}

}

add_action( 'after_setup_theme', 'cake_shop_bakery_files_setup' );

/** * Posts pagination. */
if ( ! function_exists( 'cake_shop_bakery_blog_posts_pagination' ) ) {
	function cake_shop_bakery_blog_posts_pagination() {
		$pagination_type = get_theme_mod( 'cake_shop_bakery_blog_pagination_type', 'blog-nav-numbers' );
...
function cake_shop_bakery_preloader1(){
	if(get_theme_mod('cake_shop_bakery_preloader_type', 'Preloader 1') == 'Preloader 1' ) {
		return true;
	}
	return false;
}

function cake_shop_bakery_preloader2(){
	if(get_theme_mod('cake_shop_bakery_preloader_type', 'Preloader 1') == 'Preloader 2' ) {
		return true;
	}
	return false;
}
...
function cake_shop_bakery_getpage_css($hook) {
    ...
    wp_localize_script('admin-notice-script','cake_shop_bakery',
		array('admin_ajax'	=>	admin_url('admin-ajax.php'),'wpnonce'  =>	wp_create_nonce('cake_shop_bakery_dismissed_notice_nonce')
		)
	);
	wp_enqueue_script('admin-notice-script');

    wp_localize_script( 'admin-notice-script', 'cake_shop_bakery_ajax_object',
...
function cake_shop_bakery_info_page() {
	...
	<div class="wrap about-wrap cake-shop-bakery-add-css">
			...
			<div class="feature-section three-col">
				...
				<div class="col">
					<div class="widgets-holder-wrap">
						...
						<p><a target="_blank" href="<?php echo esc_url( CAKE_SHOP_BAKERY_GET_PREMIUM_PRO ); ?>" class="button button-primary get prem">
				...
				<div class="col">
					<div class="widgets-holder-wrap">
						<h3><?php esc_html_e("Free Documentation", "cake-shop-bakery"); ?></h3>
						<p><?php esc_html_e("Our guide is available if you require any help configuring and setting up the theme. Easy and quick way to setup the theme.", "cake-shop-bakery"); ?></p>
						<p><a target="_blank" href="<?php echo esc_url( CAKE_SHOP_BAKERY_FREE_DOC ); ?>" class="button button-primary get">
							<?php esc_html_e("Free Documentation", "cake-shop-bakery"); ?>
						</a></p>
					</div>
				</div>
			</div>
		...
		<div class="cake-shop-bakery-button-container">
			<a target="_blank" href="<?php echo esc_url( CAKE_SHOP_BAKERY_GET_PREMIUM_PRO ); ?>" class="button button-primary get prem">
