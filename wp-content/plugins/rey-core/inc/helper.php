...
class Helper {
	...
	public static function get_embed_video( $args = [] ) {
		...
		if( isset($video_properties['provider']) ) {
			...
			$video = sprintf('<iframe allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" %2$s="%1$s" class="%3$s" frameborder="0" allowfullscreen></iframe>', $src, $args['attribute'], esc_attr($args['class_video']));
