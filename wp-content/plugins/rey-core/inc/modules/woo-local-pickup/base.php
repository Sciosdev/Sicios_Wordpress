...
class Base extends \ReyCore\Modules\ModuleBase {
	...
	public static function get_store_address( $return_defaults = false ){
		...
		if( $return_defaults ){
			return array_filter($store_address);
		}
	...
	public static function get_full_store_address(){
		...
		$data['maps'] = sprintf('&nbsp;<a href="%2$s" target="_blank"><u>%1$s</u></a>', esc_html__('See Google Maps', 'rey-core'), esc_url( apply_filters('reycore/local_shipping/maps_url', 'https://www.google.com/maps?q=' . implode('+', $data['store_address'])) ) );

		return apply_filters('reycore/local_shipping/address',
			sprintf('<div class="__lpickup-addr">%s %s</div>', implode(', ', $data['store_address']), $data['maps']),
			$data
		);
	}
