...
class Base extends \ReyCore\Modules\ModuleBase {
	...
	public function get_video_url(){
		...
		if( is_null($video_url) ){
			if( $product_id = $this->get_product_id() ){
				$video_url = esc_url( reycore__acf_get_field('product_video_url', $product_id ) );
			}
		}
	...
	public function setup_video_holder_for_photoswipe(){
		...
		printf('<div id="lightbox-videoHolder" class="--hidden">%s</div>', \ReyCore\Helper::get_embed_video( [
			'url'         => $video_url,
